/* fortify.h - V2.2 - All C & C++ source files to be fortified should
   #include this file  */
...
extern "C" {
#endif
   /* Function pointers for actually allocating memory.  */
       extern void *(*Fortify_real_malloc) (size_t size);
   extern void *(*Fortify_real_realloc) (void *p, size_t size);
   extern void *(*Fortify_real_calloc) (size_t num_elements, size_t size);
#ifdef FORTIFY_STRDUP
   extern char *(*Fortify_real_strdup) (const char *p);
#endif
   extern void (*Fortify_real_free) (void *p);

   /* Core Fortify Functions  */
   void *Fortify_Allocate(size_t size, unsigned char allocator,
                          const char *file, unsigned long line);
   void Fortify_Deallocate(void *uptr, unsigned char deallocator,
                           const char *file, unsigned long line);
   unsigned long Fortify_CheckAllMemory(const char *file,
                                        unsigned long line);
   unsigned long Fortify_ListAllMemory(const char *file,
                                       unsigned long line);
   unsigned long Fortify_DumpAllMemory(const char *file,
                                       unsigned long line);
   int Fortify_CheckPointer(void *uptr, const char *file,
                            unsigned long line);
   void Fortify_LabelPointer(void *uptr, const char *label,
                             const char *file, unsigned long line);
   ...
   unsigned long Fortify_GetCurrentAllocation(const char *file,
                                              unsigned long line);
   ...
   void *Fortify_realloc(void *ptr, size_t new_size, const char *file,
                         unsigned long line);
...
#ifdef FORTIFY_STRDUP
   char *Fortify_strdup(const char *oldStr, const char *file,
                        unsigned long line);
#endif
...
#ifdef __cplusplus
#include <new.h>
    /* Fortify versions of new and delete  */ void *operator new(size_t size);
...
void Fortify_PostDelete();
    ...
    /* Some compilers use a different new operator for newing arrays. * This
       includes GNU G++ (2.6.0) and Borland C++ (4.02) */
#ifdef FORTIFY_PROVIDE_ARRAY_NEW
void *operator new[] (size_t size);
void *operator new[] (size_t size, const char *file, unsigned long line);
#endif

    /* Some compilers provide a different delete operator for deleting
       arrays. * This incldues GNU G++ (2.6.0) */
...
#ifndef __FORTIFY_C__           /* Only define the macros if we're NOT in
                                   fortify.c  */
...
#else                           /* Define the special fortify functions away
                                   to nothing  */
