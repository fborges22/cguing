/* fortify.cxx - A fortified memory allocation shell - V2.2 *     This
   software  is  not public domain.  All material in * this  archive  is (C)
   Copyright 1995 Simon P.  Bullen.  The * software  is  freely
   distributable, with the condition that * no   more   than  a  nominal  fee
   is  charged  for  media. * Everything  in  this distribution must be kept
   together, in * original, unmodified form. *     The software may be
   modified for your own personal use, * but modified files may not be
   distributed. *     The  material  is  provided "as is" without warranty of
   * any  kind.  The author accepts no responsibility for damage * caused by
   this software. *     This  software  may not be used in any way by
   Microsoft * Corporation  or  its  subsidiaries, or current employees of *
   Microsoft Corporation or its subsidiaries. *     This  software  may  not
   be used for the construction, * development,  production,  or  testing of
   weapon systems of * any kind. *     This  software  may  not  be used for
   the construction, * development,  production,  or  use  of
   ...
   *     If  you use this software at all, I'd love to hear from * you.   All
   questions,  criticisms, suggestions, praise and * postcards are most
   welcome. * *            email:    sbullen@cybergraphic.com.au * *
    snail:    Simon P. Bullen *                      PO BOX 12138 *
             A'Beckett St. *                      Melbourne 3000 *
            Australia */
...
void *(*Fortify_real_malloc) (size_t size) = malloc;
void *(*Fortify_real_calloc) (size_t num_elements, size_t size) = calloc;
void *(*Fortify_real_realloc) (void *p, size_t size) = realloc;
void (*Fortify_real_free) (void *p) = free;

#ifdef FORTIFY_STRDUP
char *(*Fortify_real_strdup) (const char *p) = strdup;
#endif
 ...
 *
 */
static int st_CheckBlock(struct Header *h, const char *file,
...
static void st_OutputFortification(unsigned char *ptr, unsigned char value,
                                   size_t size);
static void st_HexDump(unsigned char *ptr, size_t offset, size_t size,
...
static int st_PurgeDeallocatedBlocks(unsigned long Bytes, const char *file,
                 ...
                 unsigned long line)
{
   ...
   if (st_Disabled) {
      ...
      if (size == 0 && (allocator == Fortify_Allocator_new
          ...
          */
         return Fortify_real_malloc(1);
      }
#endif

      return Fortify_real_malloc(size);
   }
   ...
   /* Check to see if this allocation will * push us over the artificial
      limit */
   ...
   do {
       ...
       */
      ptr = (unsigned char *) Fortify_real_malloc(FORTIFY_HEADER_SIZE
                                                  +
                                                  FORTIFY_ALIGNED_BEFORE_SIZE
                                                  + size +
                                                  FORTIFY_AFTER_SIZE);
                   ...
                   unsigned long line)
{
    ...
    */
   if (st_Disabled) {
      /* there is a possibility that this memory * block was allocated when
         Fortify was * enabled, so we must check the Allocated * list before
         we free it. */
      if (!st_IsOnAllocatedList(h)) {
         Fortify_real_free(uptr);
         return;
      } else {
         /* the block was allocated by Fortify, so we * gotta free it
            differently.
   ...
   if (!st_IsOnAllocatedList(h)) {
      ...
      if (st_IsOnDeallocatedList(h)) {
         ...
         sprintf(st_Buffer,
                 "         Memory block was deallocated by \"%s\" at %s %lu\n",
...
#endif
              st_DeallocatorName[deallocator], uptr, file, line);
      st_Output(st_Buffer);
      st_OutputDeleteTrace();
      return;
   }
#endif                          /* FORTIFY_PARANOID_DELETE * Make sure the
                                   block is okay before we free it. * If it's
                                   not okay, don't free it - it might not *
                                   be a real memory block. Or worse still,
                                   someone * might still be writing to it */
...
#endif                          /* FORTIFY_TRACK_DEALLOCATED_MEMORY * Make
                                   sure the block is being freed with a valid
                                   * deallocator. If not, complain. (but free
                                   it anyway) */
   ...
   if (st_Scope > 0) {
...
#endif                          /* FORTIFY_FILL_ON_DEALLOCATE * Begin
                                   critical region */
      FORTIFY_LOCK();
      ...
      if (st_TotalDeallocated > FORTIFY_DEALLOCATED_MEMORY_LIMIT) {
         st_PurgeDeallocatedBlocks(st_TotalDeallocated -
                                   FORTIFY_DEALLOCATED_MEMORY_LIMIT, file,
                                   line);
      }
#endif
...
#endif                          /* FORTIFY_TRACK_DEALLOCATED_MEMORY */
   {
      ...
      st_SetFortification(ptr, FORTIFY_FILL_ON_DEALLOCATE_VALUE,
                          ...
                          FORTIFY_AFTER_SIZE);
#endif                          /* FORTIFY_FILL_ON_DEALLOCATE * And do the
                                   actual free */
      Fortify_real_free(ptr);
   }
}
 ...
 */
void
Fortify_LabelPointer(void *uptr, const char *label, const char *file,
                     unsigned long line)
{
   if (!st_Disabled) {
      unsigned char *ptr = (unsigned char *) uptr
      ...
      /* copy it in */
      h->Label = (char *) Fortify_real_malloc(strlen(label) + 1);
...
/* Fortify_LeaveScope - leaves a Fortify scope level, * also prints a memory
   dump of all non-freed memory * that was allocated during the scope being
   exited. * Does nothing and returns 0 if Fortify is disabled. */
unsigned char FORTIFY_STORAGE
Fortify_LeaveScope(const char *file, unsigned long line)
{
   ...
   if (count) {
      sprintf(st_Buffer,
              "%10s %8lu bytes in %lu blocks with %lu bytes overhead\n",
...
Fortify_ListAllMemory(const char *file, unsigned long line)
{
   ...
   if (curr) {
      ...
      sprintf(st_Buffer,
              "%10s %8lu bytes in %lu blocks and %lu bytes overhead\n",
              "total", size, count, count * FORTIFY_OVERHEAD);
      st_Output(st_Buffer);
   }
...
/* Fortify_OutputStatistics() - displays statistics * about the maximum
   amount of memory that was * allocated at any one time. */
void FORTIFY_STORAGE
Fortify_OutputStatistics(const char *file, unsigned long line)
{
   ...
   sprintf(st_Buffer,
           "         Memory currently allocated: %lu bytes in %lu blocks\n",
           st_CurAllocation, st_CurBlocks);
   st_Output(st_Buffer);
   sprintf(st_Buffer,
           "         Maximum memory allocated at one time: %lu bytes in %lu blocks\n",
           st_MaxAllocation, st_MaxBlocks);
   st_Output(st_Buffer);
   sprintf(st_Buffer,
           "         There have been %lu allocations and %lu deallocations\n",
           st_Allocations, st_Frees);
   st_Output(st_Buffer);
   sprintf(st_Buffer,
           "         There was a total of %lu bytes allocated\n",
           st_TotalAllocation);
   st_Output(st_Buffer);
...
/* Fortify_GetCurrentAllocation() - returns the number of * bytes currently
   allocated. */
unsigned long FORTIFY_STORAGE
...
/* Fortify_SetAllocationLimit() - set a limit on the total * amount of memory
   allowed for this application. */
void FORTIFY_STORAGE
Fortify_SetAllocationLimit(unsigned long NewLimit, const char *file,
                           unsigned long line)
{
 ...
 * able to cope).
 */
void FORTIFY_STORAGE Fortify_Disable(const char *file, unsigned long line)
{
#ifdef FORTIFY_TRACK_DEALLOCATED_MEMORY
...
st_CheckBlock(struct Header *h, const char *file, unsigned long line)
{
...
#endif
              ptr + FORTIFY_HEADER_SIZE + FORTIFY_ALIGNED_BEFORE_SIZE,
              file, line);
      st_Output(st_Buffer);
   ...
   if (!st_CheckFortification
       (ptr + FORTIFY_HEADER_SIZE + FORTIFY_ALIGNED_BEFORE_SIZE + h->Size,
        FORTIFY_AFTER_VALUE, FORTIFY_AFTER_SIZE)) {
      sprintf(st_Buffer,
 ...
 */
static int
st_CheckDeallocatedBlock(struct Header *h, const char *file,
                         unsigned long line)
{
   unsigned char *ptr = (unsigned char *) h;
   int result = 1;
...
#endif                          /* FORTIFY_TRACK_DEALLOCATED_MEMORY *
                                   st_CheckFortification - Checks if the
                                   _size_ * bytes from _ptr_ are all set to
                                   _value_ * Returns true if all is happy. */
static int
 ...
 */
static void
st_OutputFortification(unsigned char *ptr, unsigned char value,
                       size_t size)
{
   ...
   while (offset < size) {
      ...
      if ((size - offset) < 3 * 16)
         ...
         advance = 3 * 16;
      if (advance > 0
          && st_CheckFortification(ptr + offset, value, advance)) {
         offset += advance;
         skipped = advance;
...
static void st_MakeHeaderValid(struct Header *h)
{
   h->Checksum = 0;
   h->Checksum =
       (unsigned short) (FORTIFY_CHECKSUM_VALUE - st_ChecksumHeader(h));
}
 ...
 */
static int
st_PurgeDeallocatedBlocks(unsigned long Bytes, const char *file,
                          unsigned long line)
{
 ...
 */
static int
st_PurgeDeallocatedScope(unsigned char Scope, const char *file,
                         unsigned long line)
{
 ...
 */
static void
st_FreeDeallocatedBlock(struct Header *h, const char *file,
                        unsigned long line)
{
    ...
    * Nuke out all memory that is about to be freed, including the header
    */
   st_SetFortification((unsigned char *) h,
...
static void st_OutputMemory(struct Header *h)
{
   st_HexDump((unsigned char *) h + FORTIFY_HEADER_SIZE +
              FORTIFY_ALIGNED_BEFORE_SIZE, 0, h->Size, 1);
}
 ...
 */
static void st_OutputLastVerifiedPoint()
{
   sprintf(st_Buffer,
...
static const char *st_DeallocatedMemoryBlockString(struct Header *h)
{
   ...
   return st_BlockString;
                                 ...
                                 */
#endif                          /* FORTIFY_TRACK_DEALLOCATED_MEMORY *
                                   st_DefaultOutput - the default output
                                   function */
static void st_DefaultOutput(const char *String)
{
 ...
 */
void *Fortify_realloc(void *uptr, size_t new_size, const char *file,
                      unsigned long line)
{
   unsigned char *ptr =

       (unsigned char *) uptr - FORTIFY_HEADER_SIZE -
       FORTIFY_ALIGNED_BEFORE_SIZE;
   struct Header *h = (struct Header *) ptr;
   void *new_ptr;
   ...
   if (!st_Disabled) {
      if (!uptr)
         return (Fortify_Allocate
                 (new_size, Fortify_Allocator_realloc, file, line));
      ...
      if (!st_IsOnAllocatedList(h)) {
         ...
         if (st_IsOnDeallocatedList(h)) {
            sprintf(st_Buffer,
                    ...
                    st_AllocatorName[Fortify_Allocator_realloc], file,
                    line);
            st_Output(st_Buffer);
            sprintf(st_Buffer,
                    "         Memory block %s was deallocated by \"%s\" at %s %lu\n",
      ...
      new_ptr =
          Fortify_Allocate(new_size, Fortify_Allocator_realloc, file,
                           line);
      if (!new_ptr) {
         return (0);
      }
      ...
      if (h->Size < new_size)
         memmove(new_ptr, uptr, h->Size);
      else
         memmove(new_ptr, uptr, new_size);
      ...
      if (st_IsOnAllocatedList(h)) {
         ...
         return Fortify_real_realloc(uptr, new_size);
      }
   }
}
 ...
 */
void *Fortify_calloc(size_t num, size_t size, const char *file,
                     unsigned long line)
{
   if (!st_Disabled) {
      void *ptr =
          Fortify_Allocate(size * num, Fortify_Allocator_calloc, file,
      ...
      return Fortify_real_calloc(num, size);
   }
}
...
Fortify_strdup(const char *oldStr, const char *file, unsigned long line)
{
   if (!st_Disabled) {
      char *newStr =
          (char *) Fortify_Allocate(strlen(oldStr) + 1,
      ...
      return Fortify_real_strdup(oldStr);
   }
}
 ...
 * Thanks Bjarne!
 */
Fortify_NewHandlerFunc st_NewHandler()
{
...
void *FORTIFY_STORAGE operator new(size_t size)
{
   ...
   while ((p = Fortify_Allocate(size, Fortify_Allocator_new,
                                st_AllocatorName[Fortify_Allocator_new],
                                0)) == 0) {
      if (st_NewHandler())
         (*st_NewHandler())();
      else
    ...
    operator new(size_t size, const char *file, unsigned long line)
{
   ...
   while ((p = Fortify_Allocate(size, Fortify_Allocator_new, file, line))
          == 0) {
      if (st_NewHandler())
         (*st_NewHandler())();
      else
...
void *FORTIFY_STORAGE operator new[] (size_t size) {
   ...
   while ((p = Fortify_Allocate(size, Fortify_Allocator_array_new,
                                st_AllocatorName
                                [Fortify_Allocator_array_new], 0)) == 0) {
      if (st_NewHandler())
         (*st_NewHandler())();
      else
         return 0;
...
void *FORTIFY_STORAGE
   ...
   while (
          (p =
           Fortify_Allocate(size, Fortify_Allocator_array_new, file,
                            line)) == 0) {
      if (st_NewHandler())
         (*st_NewHandler())();
      else
   ...
   return p;
                                   ...
                                   Fortify_PreDelete - C++ does not allow
                                   overloading * of delete, so the delete
                                   macro calls Fortify_PreDelete * with the
                                   source-code info, and then calls delete. */
void FORTIFY_STORAGE
 ...
 */
void FORTIFY_STORAGE Fortify_PostDelete()
{
   st_DeleteStackTop--;
...
/* Automatic log file stuff! * * AutoLogFile class. There can only ever be
   ONE of these * instantiated! It is a static class, which means that * it's
   constructor will be called at program initialization, * and it's
   destructor will be called at program termination. * We don't know if the
   other static class objects have been * constructed/destructed yet, but
   this pretty much the best * we can do with standard C++ language features. */
